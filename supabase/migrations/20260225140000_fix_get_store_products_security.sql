-- Fix: Add SECURITY DEFINER to get_store_products so anonymous visitors
-- can call it. The function does a LEFT JOIN on product_analytics which
-- has a "Deny all access" RLS policy. Without SECURITY DEFINER, the join
-- fails for unauthenticated users, returning a 400 error.
--
-- All other RPCs that touch product_analytics already use SECURITY DEFINER.
