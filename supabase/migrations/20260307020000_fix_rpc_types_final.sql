-- Fix RPC type mismatch: products.price is NUMERIC(10,2) but old RPCs used REAL.
-- Also fix visit counting to handle NULL session_ids from legacy events.
-- supabase/migrations/20260307020000_fix_rpc_types_final.sql

-- 1. Drop and recreate get_store_products with NUMERIC (no precision) to avoid REAL vs NUMERIC(10,2) mismatch
-- Final migration to ensure get_store_products types are perfectly aligned
...
-- 2. Drop and recreate get_weekly_heatmap_analytics
--    Fix: COUNT visits including those with NULL session_id (legacy events)
    ...
    RETURN (
        ...
        hourly_visits AS (
            SELECT
                ...
                -- Count distinct sessions, but also count NULLs as individual visits
                COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL)
                + COUNT(*) FILTER (WHERE session_id IS NULL AND event_type = 'VISIT')
                as visits
            FROM
                store_events
            WHERE
        ...
        total_summary AS (
            SELECT
                -- Count visits: distinct sessions + NULL session events
                (SELECT
                    COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL)
                    + COUNT(*) FILTER (WHERE session_id IS NULL)
                 FROM store_events WHERE event_type = 'VISIT'
                ) as total_visits,
